...
exports.handlePaymentIntentSucceeded = async (event) => {
  ...
  if (!payment) {
    console.error(
      `Payment not found for Tilled Payment ID: ${tilledPaymentId}`,
    );
    return;
  }
  ...
  console.log("WEBHOOK DISPATCH STARTED: ", {
    orderId: payment.orderId,
  ...
  if (metadata.billing_type === "RECURRING" && paymentMethodId) {
    console.log(
      `RECURRING payment detected for order ${payment.orderId}. Creating subscription...`,
    );
    ...
    try {
      ...
      if (!order) {
        console.error(
          `Order ${payment.orderId} not found, cannot create subscription.`,
        );
        return;
      }
